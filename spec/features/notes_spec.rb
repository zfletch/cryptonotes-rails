...
RSpec.feature 'Notes feature', :js do
  scenario 'creating a note' do
    ...
    expect(Note.last.text).not_to eq('hello world 12345')
  end
  ...
  scenario 'automatic deletion of notes' do
    ...
    expect(page).to have_content(
      'This note has been deleted from the database and can not be viewed in ' \
      'the future.',
    )
  ...
  scenario 'notes that last forever' do
    ...
    expect(page).to have_no_content('This note will be deleted when it is viewed.')
    expect(page).to have_content('Enter the Secret Key to Show the Note')
    expect(Note.count).to eq(count + 1)
  end
  ...
  scenario 'blank key' do
    ...
    expect(page).to have_no_content('Enter the Secret Key to Show the Note')
  end
